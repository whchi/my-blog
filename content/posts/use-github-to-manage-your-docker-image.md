...
4. 寫好後`docker build . -t 'local-centos7-autotagging'` 測試build(加上 -t 是讓 local 方便操作)
...
1. 建立repo
![](/images/dockerhub-create.png)
2. 選擇剛剛建立好的 repository，如果沒連接 github 帳號會要求進行連接
![](/images/dockerhub-choose-github-repo.png)
3. 連接完畢並建立 repo 即可

如有設定好連動即可自動推送並建置image，結果如圖)
![](/images/dockerhub-automatic-building.png)

之後要拉自己的環境時只需要`docker pull {username}/{repo name}:{tagname}`即可\
搭配docker-compose更方便，可參考我的**[範例](https://github.com/whchi/dockerfile/tree/pyml)**

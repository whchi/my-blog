...
## 開發
### Step1. 建立專案
`cd /path/to/your/package-develop-project && composer create-project --prefer-dist laravel/laravel . "5.5.*"` 這裡以 5.5 為例
### Step2. 建立 package 開發路徑
於 laravel documentroot 建立 packages folder\
path如下\
`packages/{vendor-name}/{package-name}/src`\
並於 src 層初始化 composer.json `composer init`，填入相關資料，以下是我的範例
{{< highlight json >}}
{
  "name": "cw/kafkalogger",

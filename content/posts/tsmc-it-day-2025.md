...
透過 notebooklm 搭配個人筆記進行的整理，只有下午場
...
應用於三大領域：品質檢測（取代人工）、製程監控與優化、設備監控（預知故障、優化維修）。

### 品質檢測的挑戰

雖易收集大量影像資料，但瑕疵影像與對應標記稀少，模型需具備泛化能力辨識新型瑕疵，且難以分析瑕疵成因。

### 視覺基礎模型 (Vision Foundation Model)

使用大量無瑕疵資料預訓練，再以少量瑕疵資料微調（Few-shot learning），可縮短訓練時間並提升效果。

### 生成式 AI (Generative AI)

使用 GAN 等技術合成訓練資料，如電路佈局轉換為工廠影像、Text-to-image 生成人工標註樣本。

### Segment Anything Model (SAM)

Meta 提出的模型支援 Zero-shot 影像分割，可依文字描述、框選或點選執行分割，台積電微調後應用於瑕疵檢測。

### Vision Language Model (VLM)

結合影像與文字分析（如 MoMo-7B），可輸出影像座標，與 SAM2 搭配提升精準度。

### 物理引導 AI (Physical Guided AI)

將物理公式融入損失函數，使模型更貼近實際、更易收斂，並支援逆向設計。
...
透過 AI 探勘與員工回報，統一定義、分類與版本控管。

### AI 協助培訓

提供個人化教練、AR/VR 模擬環境、自動生成教材。

### 未來重點

Physical AI 機器人、數位雙生 (Digital Twins)。

### 風險管理

因應 LLM 產生幻覺風險，採 RAG、歸因方法降低影響，AI 為輔助工具，決策仍以人為主。
...
好的，這是一份從您提供的來源中整理出的重點，旨在提供更清晰的理解，並加強對核心概念的掌握：
...
<https://www.tsmc.com/static/chinese/careers/2025tsmcitday/index.html>
